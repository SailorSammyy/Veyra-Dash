<template>
  <div class="min-h-screen text-white px-4 sm:px-10 py-8 sm:py-12">
    ...
    <div class="max-w-5xl mx-auto space-y-10">
      <div class="space-y-4">
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="text-xs text-white/25 mt-2">{{ filteredSources.length }}{{ search ? ` of ${activeSources.length}` : ` ${activeSources.length === 1 ? 'entry' : 'entries'}` }}</p>
          </div>
          <button
      ...
      <template v-else>
        <div
          v-if="filteredSources.length === 0 && !search"
        ...
        >
          ...
          <p class="text-white/30 text-sm font-medium">No sources yet</p>
          <p class="text-white/15 text-xs">Add one to get started</p>
        </div>

        <div
          v-else-if="filteredSources.length === 0 && search"
          class="flex flex-col items-center justify-center py-40 gap-3 rounded-2xl"
        >
          <div class="w-12 h-12 rounded-xl border border-white/10 flex items-center justify-center">
            <svg class="w-4.5 h-4.5 text-white/20" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
          ...
          <p class="text-white/30 text-sm font-medium">No results found</p>
          <p class="text-white/15 text-xs">Nothing matched "{{ search }}"</p>
        </div>
        ...
        <div v-if="filteredSources.length > 0" class="lg:hidden space-y-0">
          <div
            v-for="(source, index) in filteredSources"
            :key="source.id"
        ...
        <div v-if="deadSources.length > 0" class="space-y-3 pb-24 lg:pb-0">
          <button
            @click="showDead = !showDead"
            class="dead-section-toggle w-full flex items-center gap-3 py-2 group"
          >
            <div class="flex items-center gap-2.5 min-w-0">
              <div class="w-6 h-6 rounded-lg border border-white/[0.08] flex items-center justify-center shrink-0 bg-white/[0.02]">
                <svg class="w-3 h-3 text-white/25" fill="currentColor" viewBox="0 0 24 24">
              ...
              <span class="text-[11px] tracking-[0.2em] uppercase text-white/25 group-hover:text-white/40 transition-colors duration-150">
                Lost / Dead Source Code link
              </span>
              <span class="text-[10px] bg-white/[0.05] border border-white/[0.07] text-white/20 px-2 py-0.5 rounded-full tabular-nums">
                {{ deadSources.length }}
              </span>
            </div>
            <div class="flex-1 h-px bg-white/[0.05]"></div>
            <svg
              class="w-3.5 h-3.5 text-white/20 group-hover:text-white/40 transition-all duration-200 shrink-0"
              :class="showDead ? 'rotate-180' : ''"
          ...
          <transition name="dead-expand">
            <div v-if="showDead" class="space-y-0">

              <div class="hidden lg:block rounded-2xl border border-white/[0.04] overflow-hidden dead-table">
                <div class="grid grid-cols-[4rem_1fr_8rem_2fr_6rem] bg-white/[0.01] border-b border-white/[0.04] px-2">
                  <div class="px-3 py-3 text-[10px] tracking-[0.2em] text-white/12 uppercase">#</div>
                  <div class="px-4 py-3 text-[10px] tracking-[0.2em] text-white/12 uppercase">Title</div>
                  <div class="px-4 py-3 text-[10px] tracking-[0.2em] text-white/12 uppercase">Links</div>
                  <div class="px-4 py-3 text-[10px] tracking-[0.2em] text-white/12 uppercase">Description</div>
                  <div class="px-4 py-3 text-[10px] tracking-[0.2em] text-white/12 uppercase text-right">Actions</div>
                </div>
                <div
                  v-for="(source, index) in deadSources"
                  :key="source.id"
                  class="dead-row group grid grid-cols-[4rem_1fr_8rem_2fr_6rem] border-b border-white/[0.03] last:border-0 hover:bg-white/[0.02] transition-colors duration-150 px-2"
                >
                  <div class="px-3 py-4 flex items-center gap-2">
                    <div class="w-[2px] h-4 rounded-full bg-gradient-to-b from-white/08 to-transparent group-hover:from-white/20 transition-all duration-200 shrink-0"></div>
                    <span class="text-[11px] tabular-nums text-white/12 group-hover:text-white/25 transition-colors duration-150">
                      {{ String(index + 1).padStart(2, '0') }}
                    </span>
                  </div>
                  <div class="px-4 py-4 flex flex-col justify-center gap-1.5 min-w-0">
                    <div class="flex items-center gap-2 min-w-0">
                      <span class="text-white/30 font-medium text-sm truncate line-through decoration-white/15">{{ source.title || 'Untitled' }}</span>
                      <span class="shrink-0 inline-flex items-center gap-1 text-[9px] tracking-[0.15em] uppercase text-red-400/40 border border-red-400/15 bg-red-400/[0.04] px-1.5 py-0.5 rounded-md font-medium">
                        <span class="w-1 h-1 rounded-full bg-red-400/40 shrink-0"></span>
                        lost
                      </span>
                    </div>
                    <div v-if="source.allowed_editors?.length" class="flex items-center gap-1.5">
                      <svg class="w-3 h-3 text-white/12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Z"/>
                      </svg>
                      <span class="text-[11px] text-white/12">{{ source.allowed_editors.length }} editor{{ source.allowed_editors.length > 1 ? 's' : '' }}</span>
                    </div>
                  </div>
                  <div class="px-4 py-4 flex items-center gap-1.5">
                    <a
                      :href="source.link"
                      target="_blank"
                      title="View (may be dead)"
                      class="w-7 h-7 rounded-lg border border-white/[0.04] flex items-center justify-center text-white/15 hover:text-white/40 hover:border-white/15 transition-all duration-150"
                    >
                      <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244"/>
                      </svg>
                  ...
                  <div class="px-4 py-4 flex items-center min-w-0">
                    <p class="text-white/12 text-xs truncate group-hover:text-white/25 transition-colors duration-150">
                      {{ source.description || '—' }}
                    </p>
                  </div>
                  <div class="px-4 py-4 flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                    <template v-if="canEdit(source)">
                      <button
                        @click="openModal(source)"
                        title="Edit"
                        class="w-7 h-7 rounded-lg border border-white/[0.07] flex items-center justify-center text-white/20 hover:text-white hover:border-white/25 hover:bg-white/[0.06] transition-all duration-150"
                      >
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Z"/>
                        </svg>
                      ...
                      <button
                        @click="deleteSource(source.id)"
                        title="Delete"
                        class="w-7 h-7 rounded-lg border border-white/[0.07] flex items-center justify-center text-white/20 hover:text-red-400 hover:border-red-400/20 hover:bg-red-400/[0.05] transition-all duration-150"
                      >
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"/>
                        </svg>
              ...
              <div class="lg:hidden space-y-0">
                <div
                  v-for="(source, index) in deadSources"
                  :key="source.id"
                  class="dead-row relative py-4 border-b border-white/[0.03] last:border-0 space-y-3"
                >
                  <div class="flex items-start justify-between gap-3">
                    <div class="flex items-start gap-3 min-w-0">
                      <div class="shrink-0 flex items-center gap-2 pt-0.5">
                        <div class="w-[2px] h-4 rounded-full bg-gradient-to-b from-white/15 to-white/0"></div>
                        <span class="text-[11px] tabular-nums text-white/15 w-5">
                          {{ String(index + 1).padStart(2, '0') }}
                        </span>
                      </div>
                      <div class="min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                          <h3 class="text-white/35 font-semibold text-[15px] leading-snug line-through decoration-white/15">{{ source.title || 'Untitled' }}</h3>
                          <span class="inline-flex items-center gap-1 text-[9px] tracking-[0.15em] uppercase text-red-400/40 border border-red-400/15 bg-red-400/[0.04] px-1.5 py-0.5 rounded-md font-medium">
                            <span class="w-1 h-1 rounded-full bg-red-400/40 shrink-0"></span>
                            lost
                          </span>
                        </div>
                    ...
                    <div v-if="canEdit(source)" class="flex items-center gap-1.5 shrink-0">
                      <button
                        @click="openModal(source)"
                        class="w-8 h-8 rounded-xl border border-white/[0.06] flex items-center justify-center text-white/20 hover:text-white hover:border-white/20 hover:bg-white/[0.04] active:scale-95 transition-all duration-150"
                      >
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Z"/>
                        </svg>
                      ...
                      <button
                        @click="deleteSource(source.id)"
                        class="w-8 h-8 rounded-xl border border-white/[0.06] flex items-center justify-center text-white/20 hover:text-red-400 hover:border-red-400/20 hover:bg-red-400/[0.04] active:scale-95 transition-all duration-150"
                      >
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"/>
                        </svg>
                  ...
                  <p v-if="source.description" class="text-white/15 text-[13px] leading-relaxed line-clamp-2 pl-9">
                    {{ source.description }}
                  </p>
                  <div class="flex items-center gap-2 pl-9">
                    <a
                      :href="source.link"
                      target="_blank"
                      class="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border border-white/[0.05] text-white/20 hover:text-white/40 text-xs font-medium active:scale-95 transition-all duration-150"
                    >
                      <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244"/>
                      </svg>
...
const activeSources = computed(() => sources.value.filter(s => !s.is_dead));
const deadSources = computed(() => sources.value.filter(s => s.is_dead));

const filteredSources = computed(() => {
  const q = search.value.trim().toLowerCase();
  if (!q) return activeSources.value;
  return activeSources.value.filter(s =>
...
.dead-row { animation: fadeUp 0.15s ease both; }
...
.dead-expand-enter-active {
  transition: opacity 0.2s ease, transform 0.2s ease;
}
.dead-expand-leave-active {
  transition: opacity 0.15s ease, transform 0.15s ease;
}
.dead-expand-enter-from,
.dead-expand-leave-to {
  opacity: 0;
...
.from-white\/08 { --tw-gradient-from: rgba(255,255,255,0.08); }
</style>